            ...
            <div className="flex-grow mb-6 bg-dark-700 rounded-lg p-4 overflow-y-auto max-h-80 md:max-h-96 min-h-[200px]">
              ...
              {conversationHistory.map((msg, index) => (
                <div key={index} className={`mb-4 ${msg.role === 'user' ? 'text-right' : 'text-left'}`}>
                  ...
                  {/* Render SpeechOutput for agent messages if enabled */}
            ...
            <div className="bg-dark-700 rounded-lg p-4">
              <form onSubmit={handleFormSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-start space-x-4 mb-2">
                  <label className="text-sm cursor-pointer flex items-center text-shaga-secondary hover:text-shaga-primary transition-colors">
                    <input
                      type="checkbox"
                      checked={useSpeechInput}
                      onChange={() => setUseSpeechInput(!useSpeechInput)}
                      className="mr-2 h-4 w-4 rounded text-accent-yellow focus:ring-accent-yellow border-dark-600 bg-dark-600"
                ...
                {isClient && useSpeechInput ? (
                  <div className="p-3 bg-dark-600 rounded-lg">
                    <DynamicSpeechInput
                      ...
                      language="en-US"
                      isListening={isLoading} // Pass loading state maybe?
                      disabled={isLoading || isAudioPlaying}
                    />
                    {/* Add visual feedback for listening state here */} 
                  </div>
                ) : (
                  <textarea
                ...
                {/* Submit Button (only show if not using speech input or as fallback) */}
                {(!useSpeechInput || (useSpeechInput && isLoading)) && (
                    <button
                      type="submit"
                      className="btn-accent w-full md:w-auto"
                      disabled={isLoading || isAudioPlaying || !userInput.trim()}
                    >
