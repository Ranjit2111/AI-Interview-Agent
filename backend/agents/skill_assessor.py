...
class SkillAssessorAgent(BaseAgent):
    ...
    def _get_system_prompt(self) -> str:
        ...
        """
        return (
            f"You are an AI skill assessor specialized in evaluating skills for {self.job_role} positions. "
    ...
    def _setup_llm_chains(self) -> None:
        ...
        self.proficiency_chain = LLMChain(
            llm=self.llm,
            prompt=PromptTemplate.from_template(proficiency_template),
        ...
        self.resource_chain = LLMChain(
            llm=self.llm,
            prompt=PromptTemplate.from_template(resource_template),
            output_parser=JsonOutputParser(),
        ...
        self.profile_chain = LLMChain(
            llm=self.llm,
            prompt=PromptTemplate.from_template(profile_template),
    ...
    def _assess_recent_answer(self, context: AgentContext) -> str:
        ...
        response_parts = ["Based on your response, I've identified these skills:"]
        
        for skill, category, _ in identified:
            proficiency = self.identified_skills[skill]["proficiency"]
            response_parts.append(f"- **{skill.title()}** ({category}): {proficiency.capitalize()}")
        
        # Add suggestions
        response_parts.append("\nTo provide a more comprehensive assessment, consider:")
        response_parts.append("1. Sharing specific projects where you've applied these skills")
        response_parts.append("2. Describing your level of experience with each technology")
        response_parts.append("3. Mentioning challenges you've overcome while using these technologies")
    ...
    def _create_skill_update(self, context: AgentContext) -> str:
        ...
        # Create compact but informative feedback
        ...
        if len(recent_skills) >= 2:
            ...
            return f"I noticed you mentioned skills in {skills_str}. I'll include these in your skill profile."
